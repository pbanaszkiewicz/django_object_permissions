...
class TestPermissionViews(TestCase):
    ...
    def test_permissions_all(self):
        ...
        # anonymous user
        response = c.get(reverse("user-all-permissions", args=[user1.pk]))
        ...
        user0.save()
        response = c.get(reverse("user-all-permissions", args=[123456]))
    ...
    def test_permissions_generic_add(self):
        ...
        # anonymous user
        response = c.get(reverse("user-add-permissions", args=args))
        self.assertIn(response.status_code, [301, 302])
        ...
        response = c.get(reverse("user-add-permissions", args=(user1.pk, 'DoesNotExist')))
        self.assertEqual(404, response.status_code)

        # invalid user
        response = c.get(reverse("user-add-permissions", args=(123456, 'TestModel')))
        self.assertEqual(404, response.status_code)
        ...
        response = c.post(reverse("user-add-permissions", args=args), data)
        self.assertEqual(200, response.status_code)
    ...
    def test_permissions_generic_edit(self):
        ...
        # anonymous user
        response = c.get(reverse("user-edit-permissions", args=args))
        self.assertIn(response.status_code, [301, 302])
        ...
        # invalid class
        self.assertTrue(c.login(username=superuser.username, password='secret'))
        response = c.get(reverse("user-edit-permissions", args=(user1.pk, 'DoesNotExist', obj.pk)))
        self.assertEqual(404, response.status_code)

        # invalid user
        response = c.get(reverse("user-edit-permissions", args=(123456, 'TestModel', obj.pk)))
        self.assertEqual(404, response.status_code)

        #invalid object
        response = c.get(reverse("user-edit-permissions", args=(user1.pk, 'TestModel', 123456)))
        self.assertEqual(404, response.status_code)
        ...
        response = c.post(reverse("user-edit-permissions", args=args), data)
        self.assertEqual(200, response.status_code)
        self.assertEquals('application/json', response['content-type'])
        self.assertEqual(['Perm1'], user1.get_perms(obj))
