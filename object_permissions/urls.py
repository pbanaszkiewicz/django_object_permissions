...
urlpatterns += patterns('object_permissions.views.permissions',
    ...
    url(r'^user/(?P<user_id>\d+)/permissions/(?P<class_name>\w+)/(?P<obj_id>\d+)/?$', 'view_obj_permissions', name="user-edit-permissions"),
    url(r'^group/(?P<group_id>\d+)/permissions/(?P<class_name>\w+)/(?P<obj_id>\d+)?$', 'view_obj_permissions', name="group-edit-permissions"),
)
